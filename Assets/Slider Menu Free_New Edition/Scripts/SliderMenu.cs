...
public class SliderMenu : MonoBehaviour {
	...
	IEnumerator Upt() {
		...
		n = PlayerPrefs.GetInt("Level Completed")-1;
		ScrollStep = 1/n;
		while(true)
		{ 
				...
				if (ScrollType == scrolltype.Horizontal)
				{
					...
					SlidesContent.GetComponent<RectTransform>().sizeDelta = new Vector2(
						(PlayerPrefs.GetInt("Level Completed") + SlidesInView - 1) * (SlideSize.x + SlideMargin.y + SlideMargin.w),
					...
					{           //For Set All Slides
						...
						for (int j = 0; j < (PlayerPrefs.GetInt("Level Completed")); j++)
						{       //For Conditions. Such As Previous Slides, Active Silde, Next Slides, And Others. For Other Conditions You Can Compare i and j.
							if (HorizontalScrollbar.GetComponent<Scrollbar>().value > (ScrollStep / 2) + (i - 1) * ScrollStep && HorizontalScrollbar.GetComponent<Scrollbar>().value <= Mathf.Clamp((ScrollStep / 2) + i * ScrollStep, 0, 1))
							{
								...
								if (j == i)
								{
									...
									if (j == PlayerPrefs.GetInt("Level Completed") - 1)
					...
					if (SliderMagnet)
					{
						...
						{               //If Not Drag (In Desktop And Mobile Platform)
							for (int s = 0; s < PlayerPrefs.GetInt("Level Completed"); s++)
							{
